...
- `valh` [@R-valh], different algorithm
...
- `maposm` (not on CRAN), downloads vector objects
...
road_o <- osrmRoute(src = pt, dst = poi, osrm.profile = "bike")
road_v <- vl_route(src = pt, dst = poi, costing = "bicycle")
road_o
road_v
```
...
#| code-line-numbers: "2"
# SOTM 2025 address
pt <- geo(address = "GT-Toyota Asian Center Auditorium, Magsaysay Avenue", method = "osm")
# Transform dataframe with X/Y coords to an sf object
(pt <- st_as_sf(pt, coords = c("long", "lat"), crs = 4326))
```
